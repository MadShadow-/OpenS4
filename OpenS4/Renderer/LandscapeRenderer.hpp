...
class LandscapeRenderer {
   ...
   private:
    bool isInTriangle(const glm::vec2& p, const glm::vec2& p0,
        ...
        auto A = -p1.y * p2.x + p0.y * (p2.x - p1.x) + p0.x * (p1.y - p2.y) +
                 p1.x * p2.y;
    ...
    BarycentricFactors getBarycentric(const glm::vec2& pixel,
                                      const glm::vec2& _1, const glm::vec2& _2,
                                      const glm::vec2& _3) {
   ...
   public:
    glm::vec2 toModelPositionHighPrecision(glm::vec2 pixel) {
        ...
        glm::vec2 neighbors[] = {glm::vec2(0, -1), glm::vec2(1, -1),
        ...
        if (x >= 0 && x < m_landscape->getWidth() - 1 && y > 0 &&
            ...
            do {
                ...
                for (glm::vec2 neighbor : neighbors) {
                    ...
                    if (xN < maxX && xN >= minX && yN < maxY && yN >= minX) {
                        glm::vec2 neigh = toPixelPosition(glm::vec2(xN, yN));
            ...
            int fX = x;
            int fY = y;
            for (auto y = std::max(fY - 5, minY); y < std::min(fY + 5, maxY);
                 y++) {
                for (auto x = std::max(fX - 5, minX);
                     x < std::min(fX + 5, maxX); x++) {
                    ...
                    if (isInTriangle(pixel, _1, _2, _3)) {
                        ...
                        auto x_ = factors.w1 * (x + 1) + factors.w2 * (x + 1) +
                                  factors.w3 * x;
    ...
    glm::vec2 screenToWorld(glm::vec2 screen) {
        glm::vec2 clip(2 * screen.x / m_screenWidth - 1.0,
                       1 - (2 * screen.y) / m_screenHeight);
        ...
        glm::vec4 rayEye =
            glm::inverse(m_transformation->getProjectionMatrix()) * ray;

        rayEye = glm::vec4(rayEye.x, rayEye.y, -1.0, 0);

        glm::vec3 rayWorld = glm::vec3(
            glm::inverse(m_transformation->getModelViewMatrix()) * rayEye);
    ...
    void setTransform(OpenS4::Renderer::TransformationPipeline* transform,
                      int width, int height) {
        m_transformation = transform;
        m_screenHeight = height;
        m_screenWidth = width;

        m_tr = screenToWorld(glm::vec2(m_screenWidth, 0));
        m_bl = screenToWorld(glm::vec2(0, m_screenHeight));

        m_tr = toModelPosition(m_tr);
        m_bl = toModelPosition(m_bl);

        m_tl = screenToWorld(glm::vec2(0, 0));
        m_br = screenToWorld(glm::vec2(m_screenWidth, m_screenHeight));
    ...
    bool isVisibleModel(glm::vec2 model) {
        ...
        if (y >= visibleVolumeLowerBound.y && y <= visibleVolumeUpperBound.y) {
            if (x >= visibleVolumeLowerBound.x &&
                x <= visibleVolumeUpperBound.x)
                return true;
        }
