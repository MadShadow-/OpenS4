...
namespace OpenS4::Import
{
    class ImageData
    {
       ...
       public:
        ...
        __inline void setPixel(uint32_t x, uint32_t y, uint32_t color)
        {
            m_data[getIndex(x, y)] = color;
        }

        __inline void setPixel(uint32_t idx, uint32_t color) { m_data[idx] = color; }

        auto getHeight() const { return m_height; }
        auto getWidth() const { return m_width; }

        __inline auto getColor(uint32_t x, uint32_t y) const
        {
    ...
    {
       protected:
        ImageData readImage(std::shared_ptr<Reader> reader,
                            uint32_t offset,
                            ...
                            uint32_t paletteOffset)
        {
            ...
            ImageData imgData(
                width, height, paletteOffset, left, top, flag1, flag2);
                ...
                while (j < width * height)
                {
                    ...
                    if (value <= 1)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                OpenS4::getLogger().warn(
                                    "Position exceeded size of image at offset "
                                    "%d",
                                    offset);
                                print_error = false;
                            }
                    ...
                    if (value >= first_player_color)
                    {
                        ...
                        {
                            auto f =
                                [](float a, float b, float c, float d, int v) {
                                    return c + ((d - c) / (b - a)) * (v - a);
                                };

                            // Scale player color to a range of "transparency"
                            // exclude 0 and 255 because 0 means transparent
                            // which is needed for transparent pixels and 255 is
                            // needed for black itself
                            color = 0x00000000;
    ...
    class GraphicsLazy : public IGraphics
    {
       ...
       public:
                     ...
                     bool ignoreLastLine)
        {
            ...
            m_gil = gil;
            m_pal = pal;
            m_ignoreLastLine = m_ignoreLastLine;
        }
        ...
        ImageData getImage(int offset) override
        {
            ...
            auto image = readImage(
                m_reader, gfx_offset, m_pal, m_pal->get_offset(palette));

            if (m_ignoreLastLine)
            {
                for (auto x = 0; x < image.getWidth(); x++)
                    image.setPixel(x, image.getHeight() - 1, 0);
            }
        ...
        {
            {
                ...
                while (size != 0)
                {
                    ...
                    auto images =
                        readImages(reader, image_type, row_count, offset);

                    m_images.insert(
                        m_images.end(), images.begin(), images.end());
